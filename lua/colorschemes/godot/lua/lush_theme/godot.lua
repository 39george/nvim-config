...
local raisin_black        = hsl(225, 6, 14)
local jet                 = hsl(216, 5, 18)
...
local crayole             = hsl("#ff7085")
local persian_pink        = hsl("#ff8ccc")
...
return lush(function(injected_functions)
  ...
  return {
    ...
    NonText { fg = jet.li(15) },    -- characters that don't exist in the text
    ...
    StatusLine { bg = raisin_black },
    StatusLineNC { fg = fadefg, bg = raisin_black.da(1) },

    TabLine { fg = fg.da(30), bg = mid }, -- not active tab page label
    TabLineFill { bg = raisin_black },    -- where there are no labels
    ...
    MsgSeparator { fg = crayole },        -- Separator for scrolled messages `msgsep` flag of 'display'
    ...
    SpellBad { sp = crayole, gui = "underline" },
    SpellCap { fg = magenta, gui = un },
    SpellLocal { fg = crayole, gui = un },
    ...
    Statement { fg = crayole }, -- (preferred) any statement
    Conditional { fg = persian_pink },
    Repeat { Statement },
    ...
    PreProc { fg = crayole }, --  generic Preprocessor
    ...
    Special { fg = crayole },    -- (preferred) any special symbol
    ...
    sym "@variable.parameter" { fg = cool_gray },
    sym "@variable.builtin" { fg = crayola, gui = it }, -- Variable names that are defined by the languages like `this` or `self`.

    sym "@function" { Function },
    sym "@function.builtin" { Function },
    sym "@function.macro" { fg = coral }, -- macro defined functions: each `macro_rules` in Rust
    sym "@method" { Function },
    sym "@constructor" { Function },     -- For constructor: `{}` in Lua and Java constructors.
    ...
    sym "@text.title" { fg = crayole },        -- Text that is part of a title
